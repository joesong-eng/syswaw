...
@section('content')
    <div class="my-5 p-2 shadow">
        ...
        <div class="text-right pb-3">
            ...
            <x-primary-button onclick="triggerBroadcast()" style="background-color: cadetblue;">Trigger Broadcast</x-primary-button>
        </div>
    </div>
    <div class="my-5 p-2 shadow">
        <p>
        ...
        <div class="text-right">
            ...
            <button id="status-tcp" onclick="triggerRedis('status')" class="p-1 rounded-md bg-yellow-300 dark:bg-yellow-500 text-gray-900 dark:text-gray-800">
                {{ __('msg.status') }}
            </button>
            <button id="restart-tcp" onclick="triggerRedis('restart')" class="p-1 rounded-md bg-yellow-300 dark:bg-yellow-500 text-gray-900 dark:text-gray-800">
                {{ __('msg.restart') }}
            </button>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        window.triggerBroadcast = function() {
            const postData = { action: 'By Broadcast' };
            fetch('/admin/ethereal/broadcast', {
                method: 'POST',headers: {'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,'Content-Type': 'application/json','Accept': 'application/json'},body: JSON.stringify(postData)
        ...
        window.triggerEvent = function() {
            const postData = {action: 'By Event'};
        ...
        const xstate={'stopped':'已停止 ❌','running':'已啟動 ✅','restarting':'已重新啟動 ✅'}
        document.addEventListener('DOMContentLoaded', () => {
            ...
            .listen('.tcp.status.updated', (e) => {
                ...
                document.getElementById('redis_msg').innerText = `TCP 狀態：${e.status}，動作：${e.action}`; // 修改顯示內容
            })
        })
