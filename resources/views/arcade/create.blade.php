...
@section('content')
@php
    $route = Auth::user()->hasRole('admin') ? 'admin.arcade.store' : 'arcade.store';
@endphp
<div class="mx-auto max-w-md">
    <div class="shadow-lg rounded-lg p-4">
        <div class="py-2 px-3">
            <h4 class="font-semibold text-md text-gray-800 dark:text-gray-200 leading-tight">{{__('msg.create_arcade')}}</h4>
            <small class="text-gray-600 dark:text-gray-300">A請輸入商鋪的詳細資訊,同時將自動註冊老闆管理帳號</small>
        </div>
        <hr>
        <form action="{{ route($route) }}" method="POST">
            ...
            <div class="mt-4">
                <x-label for="arcadeKey" class="text-gray-700 dark:text-gray-300">{{ __('msg.token') }}:</x-label>
                <x-input id="arcadeKey" class="input-st w-full" type="text" name="arcadeKey" value="{{ $arcadeKey ?? '' }}" required autofocus autocomplete="arcadeKey" onpaste="handlePaste(event)" />
            </div>
            @else
            <input id="arcadeKey" class="input-st w-full" type="text" name="arcadeKey" value="{{ $arcadeKey ?? '' }}" readonly/>
            @endif
            
            <div class="mt-4"><!-- 商鋪名稱欄位 -->
                <x-label for="arcade_name" class="text-gray-700 dark:text-gray-300">{{__('msg.name')}}:</x-label>
                <x-input id="arcade_name" class="w-full" type="text" name="arcade_name" required />
            </div>
            <div class="mt-4"><!-- 商鋪地址欄位 -->
                <x-label for="address" class="block text-gray-700 dark:text-gray-300">{{__('msg.address')}}:</x-label>
                <x-input type="text" id="address" name="address" class="w-full" />
            </div>
            <div class="mt-4"><!-- 商鋪電話欄位 -->
                <x-label for="phone" class="block text-gray-700 dark:text-gray-300">{{__('msg.phone')}}:</x-label>
            ...
            <div class="mt-4">
                ...
                @if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('machine-owner'))
                    <label class="block text-gray-700 dark:text-gray-300">{{__('msg.arcade')}}{{__('msg.type')}}：</label>
                    <div>
                        <label class="inline-flex items-center">
                            <input type="radio" id="type_physical" name="type" value="physical" class="input-st" required>
                            <span class="ml-2">{{__('msg.physical_arcade')}}</span>
                        </label>
                    </div>
                    <div>
                        <label class="inline-flex items-center">
                            <input type="radio" id="type_virtual" name="type" value="virtual" class="input-st" required>
            ...
            @guest
                ...
                <div class="mt-4"><!-- 店主帳戶欄位 -->
                    ...
                    <x-input id="user-name" class="w-full" type="text" name="user_name" required autofocus autocomplete="name" />
                </div>
                <div class="mt-4"><!-- 店主郵件欄位 -->
                    <x-label for="email" value="{{ __('Email  - 店主郵箱 -') }}" class="text-gray-700 dark:text-gray-300"/>
                    <x-input id="email" class="w-full" type="email" name="new_user_email" required autocomplete="username" />
                </div>
                ...
                <div class="mt-4"><!-- 店主密碼確認欄位 -->
                    ...
                    <x-input id="password_confirmation" class="w-full" type="password" name="password_confirmation" required autocomplete="new-password" />
                </div>
                <div class="py-4">
...
<script>
    function handlePaste(event) {
        ...
        // 定義要移除的前綴
        const prefixToRemove = 'https://sys.tg25.win/arcades/';
