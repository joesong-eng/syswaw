                        ...
                        @if (session('reportData'))
                            ...
                            <div class="mt-1 text-sm text-gray-600 space-y-1">
                                @if (session('dateRange'))
                                    <p>{{ session('dateRange.start') }} ~ {{ session('dateRange.end') }}</p>
                                @endif
                                {{-- @if (session('dateRange.start_time') && session('dateRange.end_time'))
                                        <p>詳細時間：{{ session('dateRange.start_time') }} ~
                                            {{ session('dateRange.end_time') }}</p>
                                    @endif --}}
                            ...
                            <div class="overflow-y-auto max-h-[calc(100vh-150px)]">
                                <table
                                    ...
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-2 py-0 sticky top-0 bg-gray-100 z-1 w-[20%]">
                                                機台
                                            </th>
                                            <!-- 場地與廠商（合併為一個 th） -->
                                            ...
                                            {{-- 投幣開分 --}}
                                            <th colspan="2"
                                                class="px-2 py-0 justify-end sticky top-0 bg-blue-100 z-1 w-[20%]">
                                                <div
                                                    class="flex flex-col lg:flex-row space-y-2 lg:space-y-0 lg:space-x-2 ">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <template x-for="(data, index) in displayedData" :key="index">
                                            <tr>
                                                <td class="px-0 py-2 md:px-2  text-sm text-gray-900 text-center truncate"
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td class="px-1 py-2 text-left text-sm font-bold text-gray-700 sticky bottom-0 bg-gray-100 z-10"
                                                colspan=3 {{-- colspan="{{ empty(session('filters.arcade_id')) && empty(session('filters.owner_id')) ? 3 : (empty(session('filters.arcade_id')) || empty(session('filters.owner_id')) ? 2 : 1) }}" --}}>
                                                頁面總計
                                            </td>
