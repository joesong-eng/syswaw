...
@section('content')
    ...
    <div class="flex justify-center bg-gray-100 dark:bg-gray-900 pt-2"
        x-data="{ editChipModal: false, selectedChip: {}, addOwnerModal: false }">
        <div class=" w-full max-w-6xl bg-white bg-opacity-60 dark:bg-gray-800 dark:bg-opacity-70 shadow-lg rounded-lg p-2" >
            <!-- 篩選機主 -->
            {{-- <form action="{{ route('printKeys') }}" method="POST" id="printForm">
                @csrf --}}
                <!-- 篩選與列表標題區塊保持不變 -->
                <div class="relative pt-4 bg-white dark:bg-gray-700 rounded-lg shadow hover:bg-gray-50 dark:hover:bg-gray-600 overflow-scroll ">
                    <!-- 標題與按鈕 -->
                    <div class="flex justify-between">
                        ...
                        <div class="flex w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-1">
                            <!-- 新增金鑰按鈕 (小螢幕靠右) -->
                            <button name="elem2-2" class="bg-blue-500 order-2 w-full sm:w-auto sm:ml-auto max-w-40 px-4 py-2 text-white rounded-md hover:bg-blue-600"
                                ...
                                <form name="elem3" class="w-full"
                                    ...
                                    <select name="owner_id" id="owner_id" 
                                        ...
                                        @foreach ($storeOwners as $owner)
                                            <option value="{{ $owner->id }}" {{ $filterOwner == $owner->id ? 'selected' : '' }}>
                                                {{ $owner->name }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <!-- 列印按鈕與列表標題 -->
                    <div class="flex items-center border-b border-gray-200 text-sm font-medium text-gray-700 dark:text-gray-100 px-1">
                        <button type="button" id="printButton" class="w-[6%] qrcodeprint px-1 text-center text-xs items-center rounded-lg p-2 bg-yellow-200 dark:bg-yellow-800 hover:bg-yellow-300" onclick="handlePrintClick()">
                            列印
                        </button>
                        <div class="w-[20%] px-1 text-center text-sm">id {{__('msg.token')}}</div>
                        <div class="w-[10%] px-1 text-sm">{{__('msg.expires_at')}}</div>
                        <div class="w-[14%] px-1 text-center text-sm">{{__('msg.arcade')}}</div>
                        <div class="w-[22%] px-1 text-center text-sm">{{__('msg.owner')}}|{{__('msg.creator')}}</div>
                        <div class="w-[12%] px-1 text-center text-sm">{{__('msg.status')}}</div>
                        <div class="w-[16%] px-1 text-center text-sm">{{__('msg.actions')}}</div>
                    </div>
                    <!-- 列表資料部分 -->
                    <div class="shadow-lg rounded-sm bg-gray-200 dark:bg-gray-700" style="max-height: calc(100vh - 180px);">
                        @foreach ($machines as $machine)
                            <div class="flex items-center border-b border-gray-200 text-sm font-thin text-gray-700 dark:text-gray-100 space-x-1">
                                <div class="w-[6%] break-words text-center">
                                    <input type="checkbox" name="selected_ids[]" value="{{ $machine->id }}" class="form-checkbox h-5 w-5 text-blue-600">
                                </div>
                                <div class="w-[20%] break-words cursor-copy text-center" onclick="copyToClipboardAndGenerateLink('{{ $machine->token }}','chip')">
                                ...
                                <div class="w-[16%] break-words text-right flex space-x-1">
                                    ...
                                    <button type="button" class="inline-block bg-blue-500 text-white rounded-md hover:bg-blue-600 transition" 
                                    ...
                                    <form action="{{ route('stores.destroyKey', $machine) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-500 text-white rounded-md hover:bg-red-600 transition p-1"
        ...
        <!-- 新增機器金鑰模態框 -->
        <x-modal.machine-key 
            :storeOwners="$storeOwners" :stores="$stores"  :currentUser="$users" 
        />
        <x-modal.chip-key :stores="$stores" :vsStores="$vsStores" />
    </div>
@endsection
@php
    $title = __('msg.chip_token').__('msg.management');
@endphp
