...
@section('content')
    <x-slot name="header">
        ...
        @if (session('success'))
            <div id="success-message" class="message mb-4 p-4 text-green-800 bg-green-100 border border-green-200 rounded-lg"> {{ session('success') }} </div>
        @endif
        @if ($errors->any())
            <div id="errors_msg" class="mb-4 p-4 text-red-800 bg-red-100 border border-red-200 rounded-lg">
                <ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>
                <button onclick="document.getElementById('errors_msg').remove();" class="absolute top-0 bottom-0 right-0 px-4 py-3 text-yellow-700 hover:text-yellow-900">X </button>
            </div>
        @endif
    </x-slot>
