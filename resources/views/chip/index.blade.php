        ...
        <div class="relative w-full bg-white bg-opacity-60 dark:bg-gray-900 dark:bg-opacity-70 shadow-lg rounded-lg">
            <div class="flex justify-between items-center mb-2 px-6 pt-6">
                ...
                <div class="flex items-center">
                    <form id="filterForm" action="{{ route('chips.index') }}" method="GET" class="mr-4">
                        <select name="filter" id="filter" class="border rounded-md px-2 py-1" onchange="document.getElementById('filterForm').submit();">
                            ...
                            <option value="unused" {{ $filter == 'unused' ? 'selected' : '' }}>{{ __('msg.unused') }}</option>
                        </select>
                    </form>
                    <form action="{{ route('chip.store') }}" method="POST" class="confirm-submit">
                        @csrf
                        <x-button>
                            {{ __('msg.add_token') }}
                        </x-button>
                ...
                <div class="overflow-y-auto max-h-[calc(100vh-200px)] sm:max-h-[calc(100vh-150px)]">
                    @foreach ($chipKeys as $chipKey)
                        <div class="flex items-center my-4 mx-0 border-b border-gray-200 text-sm font-medium text-gray-700 dark:text-gray-100">
                            ...
                            <div class="w-[33%] px-1 break-words cursor-copy" id="chipKey" onclick="copyToClipboardAndGenerateLink('{{ $chipKey->key }}', 'chip')">
                                {{ $chipKey->key }}
                            </div>
                            ...
                            <div class="w-[11%] px-1 text-center">
                                <form action="{{ route('chip.destroy', $chipKey->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="w-[9%] break-words text-center">
                                <input type="checkbox" name="selected_ids[]" value="{{ $chipKey->id }}" class="form-checkbox h-5 w-5 text-blue-600">
                            </div>
                        </div>
                    @endforeach
                </div>
