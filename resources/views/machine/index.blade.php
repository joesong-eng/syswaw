            ...
            <div class="container mx-auto px-2 pb-3">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                    ...
                    <div class="flex items-start border-b border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-100 shadow-lg">
                        ...
                        <div class="w-full max-w-[100px] text-center px-1">{{ __('msg.name') }}/<a style="font-size: xx-small;text-overflow: ellipsis;">{{ __('msg.creator') }}</a></div>
                    ...
                    @foreach ($machines as $machine)
                        <div class="flex items-center border-b border-gray-200 text-sm font-medium text-gray-700 dark:text-gray-100 py-1">
                            ...
                            <div class="w-full max-w-[100px] px-1 break-words m-auto text-center">
                                {{ $machine->name }}
                                <div class="font-thin text-end pe-1" style="font-size: xx-small; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    {{ $machine->creator->name ?? 'Unknown' }}
                                </div>
                            </div>
                            <div class="w-[calc(3/11*(100%-216px))] px-1 break-all m-auto text-center">{{ $machine->arcade->name ?? 'Unknown' }}</div>
                            <div class="w-[calc(3/11*(100%-216px))] px-0 break-all m-auto text-center">{{ $machine->owner->name ?? 'Unknown' }}</div>
                            <div class="w-[calc(3/11*(100%-216px))] px-1 break-all m-auto justify-center">{{ __('msg.'.$machine->machine_type) ?? 'Unknown' }}</div>
                            <div class="w-[calc(2/11*(100%-216px))] px-0 break-all text-center">{{ $machine->chip->id ?? 'N/A' }}</div>
                            <div class="w-[105px] flex items-center justify-center space-x-1 m-auto">
                                <!-- 啟用/停用 -->
                                <form action="{{ route('machine.toggleActive', $machine->id) }}" method="POST" class="flex">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="is_active" value="{{ $machine->is_active ? 0 : 1 }}">
                                    <button type="submit" onclick="return confirm('{{ __('msg.confirm_active') }} {{ $machine->is_active ? __('msg.deactivate') : __('msg.activate') }}{{ __('msg.this_machine') }}{{ __('msg.zh_ask') }}')" class="m-auto text-white transition p-1">
                                ...
                                <form action="{{ route('machine.destroy', $machine->id) }}" method="POST" class="m-0">
                                    @csrf
                                    @method('DELETE')
                                    <x-button class="bg-red-500 text-white rounded-md hover:bg-red-600 transition !p-1 !m-0 !my-auto" type="submit" onclick="return confirm('{{ __('msg.confirm_delete') }}{{ __('msg.zh_ask') }}')">
