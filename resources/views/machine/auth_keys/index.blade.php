...
@section('content')
    <div class="container mx-auto px-1 sm:px-6 lg:px-8 py-4 ">
        <div class="flex justify-end items-center mb-6 px-1">
            ...
            <form action="{{ route('machine.auth_keys.store') }}" method="POST" class="flex items-center confirm-submit"
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="flex flex-col h-[calc(100vh-230px)] sm:h-[calc(100vh-152px)]">
                    ...
                    <div class="bg-white rounded-lg shadow-lg">
                        ...
                        <div
                            ...
                            <div class="w-[25%] px-1 text-base break-words items-center text-center">
                                <a href="{{ route('machine.auth_keys.index', ['sort' => 'auth_key', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc']) }}"
                            ...
                            <div class="w-[18%] px-1 text-base break-words items-center text-center">
                                <a href="{{ route('machine.auth_keys.index', ['sort' => 'chip_hardware_id', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc']) }}"
                                    class="hover:underline">
                            ...
                            <div class="w-[13%] px-1 text-base break-words items-center text-center">
                                <a href="{{ route('machine.auth_keys.index', ['sort' => 'expires_at', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc']) }}"
                            ...
                            <div class="w-[15%] px-1 text-base break-words items-center text-center">
                                <a href="{{ route('machine.auth_keys.index', ['sort' => 'status', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc']) }}"
                                    {{-- Corrected route --}} class="hover:underline">
                                    {{ __('msg.status') }}
                ...
                <div class="DataRows flex-grow overflow-y-auto">
                    @forelse ($machineAuthKeys as $key)
                        <div
                            class="flex items-center border-b border-gray-200 text-sm text-gray-700 hover:bg-gray-50">
                            <div id="auth_key" class="w-[4%] px-1 text-xs break-words items-center text-center">
                                {{ $key->id }}
                            </div>
                            ...
                            <div class="w-[13%] px-1 py-4 c-list break-words text-center text-gray-500">
                                {{ $key->expires_at ? $key->expires_at->format('ymd H:i') : '___("msg.never")' }}
                            </div>
                            <div class="w-[15%] px-1 py-4 text-center break-words">
                                @if ($key->status === 'active')
                                    <span
                                        class="px-2 text-sm inline-flex text-base leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                        {{ __('msg.active') }}
                                    </span>
                                @elseif($key->status === 'inactive')
                                    <span
                                        class="px-2 text-sm inline-flex text-base leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                        {{ __('msg.inactive') }}
                                    </span>
                                @else
                                    <span
                                        class="px-2 text-sm inline-flex text-base leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                        {{ __('msg.pending') }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="w-[8%] px-1 py-4 whitespace-nowrap text-center font-medium">
                                {{-- 編輯按鈕 --}}
                                {{-- <a href="{{ route('machine.auth_keys.edit', $key) }}" {{-- *** 編輯按鈕連結 *** --}}
                                ...
                                @if ($key->status == 'pending' && $key->machine_id == null)
                                    {{-- 只允許刪除 pending 且未綁定的 --}}
                                    <form action="{{ route('machine.auth_keys.destroy', $key->id) }}" method="POST"
            ...
            {{-- 新增：隱藏的表單，用於提交列印請求 --}}
            <form id="printForm" action="{{ route('machine.auth_keys.print') }}" method="POST" style="display: none;">
                @csrf
