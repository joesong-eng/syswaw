...
@section('content')
    ...
    <div class="flex justify-center bg-gray-100 dark:bg-gray-900 " 
        ...
        <div class="w-full max-w-2xl bg-white bg-opacity-60 dark:bg-gray-800 dark:bg-opacity-70 shadow-lg rounded-lg">
            <div x-show="showHeader" class="relative bg-gray-200 dark:bg-gray-800 shadow">
                @if (session('success'))
                    <button @click="showHeader = false" class="absolute top-0 right-0 p-2 text-gray-600 dark:text-gray-300">
            ...
            <!-- 頂部：標題和新增按鈕 -->
            {{-- <div class="flex items-center text-lg font-medium text-gray-700 dark:text-gray-300">
                    ...
                    <div class="flex justify-end w-full sm:w-auto">    <!-- 篩選框 -->
                        <form method="GET" class="w-full px-2" action="{{ route('machine.machines') }}">
                            <div class="relative inline-block">
                                <button type="button" 
                                    class="dropdown-btn w-32 py-2 px-4 border rounded-md text-gray-700 dark:text-gray-300
                                ...
                                <div x-cloak x-show="storeType" class="dropdown-content w-full bg-white shadow-lg rounded-lg mt-2 absolute z-10 border border-gray-300">
                                    ...
                                    hover:bg-blue-100 dark:hover:text-gray-700
                                    ">
...
@endforeach
        ...
        <div x-cloak x-show="addMachineModal" class="fixed inset-0 z-50">
            ...
            <div class="relative w-full h-full flex items-center justify-center p-4">
                <!-- 模態框內容 -->
                <div class="p-6 pb-10 bg-white dark:bg-gray-800 w-full max-w-md rounded-lg shadow-lg" @click.away="addMachineModal = false">
                    <h3 class="text-lg font-medium text-gray-700 dark:text-gray-300 mb-4">新增機器</h3>
                    <form action="{{ route('machine.addMachine') }}" method="POST">
                        ...
                        <!-- 金鑰 -->
                        <div class="mb-4">
                            <label for="token" class="block text-sm text-gray-600 dark:text-gray-400">金鑰</label>
                            <input type="text" id="token" name="token" class="w-full mt-1 p-2 border rounded-md" required>
                        </div>
                        <!-- 名稱 -->
                        <div class="mb-4">
                            <label for="mname" class="block text-sm text-gray-600 dark:text-gray-400">名稱</label>
                            <input type="text" id="mname" name="mname" class="w-full mt-1 p-2 border rounded-md" required>
                        </div>

                        <!-- 機器類型 -->
                        <div class="mb-4">
                            <label for="machine_type" class="block text-sm text-gray-600 dark:text-gray-400">機器類型</label>
