...
@if (Auth::user()->hasRole('admin'))
    <x-sidebar.info-card>
        <div class="text-center">
            <div class="text-lg font-medium text-gray-900 break-words">
    ...
    <x-modal.sidebar-link route="admin.reports.index" icon="Floppy" title="{{ __('報表系統') }}"
        description="生成和查看机器数据报表" />
        ...
        <x-sidebar.info-card>
            ...
            <div class="space-y-1 text-sm">
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.name') }}:</span> <span
                        class="text-gray-800 truncate text-right">{{ $ownerArcade->name }}</span>
                </div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.address') }}:</span> <span
                        class="text-gray-800 truncate text-right">{{ $ownerArcade->address }}</span>
                </div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.phone') }}:</span> <span
                        class="text-gray-800 text-right">{{ $ownerArcade->phone }}</span></div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.business_hours') }}:</span>
                    <span
                        class="text-gray-800 truncate text-right">{{ $ownerArcade->business_hours ?? __('msg.not_set') }}</span>
                </div>
                <div class="flex justify-between items-center"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.authorization_code') }}:</span>
                    <code
                        class="text-xs bg-gray-100 text-gray-800 px-1 py-0.5 rounded">{{ $ownerArcade->authorization_code ?? __('msg.not_set') }}</code>
                </div>
            </div>
        </x-sidebar.info-card>
    @else
        <x-sidebar.info-card class="text-center text-red-600 font-semibold">
...
@elseif (Auth::user()->hasRole('arcade-staff'))
    ...
    @if ($staffOwnerArcade)
        <x-sidebar.info-card>
            ...
            <div class="space-y-1 text-sm">
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.arcade') }}:</span> <span
                        class="text-gray-800 truncate text-right">{{ $staffOwnerArcade->name }}</span>
                </div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.address') }}:</span> <span
                        class="text-gray-800 truncate text-right">{{ $staffOwnerArcade->address }}</span>
                </div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.phone') }}:</span> <span
                        class="text-gray-800 text-right">{{ $staffOwnerArcade->phone }}</span></div>
                <div class="flex justify-between"><span
                        class="font-bold text-gray-600 pr-2">{{ __('msg.owner') }}:</span> <span
                        class="text-gray-800 truncate text-right">{{ $staffOwnerArcade->owner->name }}</span>
                </div>
            </div>
        </x-sidebar.info-card>
    @endif
    ...
    <x-sidebar.info-card>
        ...
        <div class="space-y-1 text-sm">
            <div class="flex justify-between">
                <span class="font-bold text-gray-600 pr-2">{{ __('msg.machine_owner') }}:</span>
...
@elseif (Auth::user()->hasRole('machine-staff'))
    <x-sidebar.info-card>
        ...
        <div class="space-y-1 text-sm">
            <div class="flex justify-between">
                <span class="font-bold text-gray-600 pr-2">{{ __('msg.machine_owner') }}:</span>
                <span class="text-gray-800">{{ Auth::user()->parent->name ?? __('無店主資料') }}</span>
            </div>
        </div>
    </x-sidebar.info-card>
