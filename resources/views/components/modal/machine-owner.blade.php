...
<div id="addOwnerModal" class="flex hidden fixed inset-0 z-50 items-center justify-center bg-black bg-opacity-50" >
    <div class="relative w-full max-w-md bg-white rounded-lg shadow-lg dark:bg-gray-800">
        ...
        <div class="flex justify-between items-center px-4 py-3 border-b dark:border-gray-600">
            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">新增機主</h3>
            <button class="text-gray-500 hover:text-gray-800 dark:hover:text-gray-300" onclick="toggleModal('addOwnerModal')">
                X
            </button>
        </div>
        <!-- 模態框內容 -->
        <form action="{{ route('stores.addMachineOwner') }}" method="POST" class="p-6">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">名稱</label>
