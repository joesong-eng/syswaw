<div x-cloak x-show="createRoleModal" class="fixed inset-0 z-50">
    ...
    <div class="relative w-full h-full flex items-center justify-center p-4">
        <div class="relative bg-white w-full max-w-md rounded-lg shadow-lg" @click.away="createRoleModal = false">
            ...
            <div class="p-6">
                <form action="{{ route('admin.roles.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="hidden" id="slug" name="slug" value="web" required
                            class="mt-1 block w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300 transition">
                    </div>
                    <div class="flex items-center justify-end space-x-3 mt-6">
                        <button type="button" @click="createRoleModal = false"
                            class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 transition">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700 transition">Create</button>
                    </div>
                </form>
            </div>
        </div>
