<div x-cloak x-show="editChipModal" class="fixed inset-0 z-50">
    ...
    <div class="relative w-full h-full flex items-center justify-center p-4">
        <div class="p-6 relative bg-white dark:bg-gray-800 w-full max-w-md rounded-lg shadow-lg" @click.away="editChipModal = false">
            <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-100">{{__('msg.edit')}}{{__('msg.chip')}}{{__('msg.machine')}}</h2>
            ...
            <form x-bind:action="`{{ route('admin.connectMachineUpdate') }}`" method="POST">
                ...
                <div class="w-full mb-3">
                    @if(Auth::user()->hasRole('store-owner'))
                        @if(Auth::user()->storeable_type === 'App\Models\Store')
                            <input type="hidden" name="arcade_id" value="{{ \App\Models\Store::find(Auth::user()->storeable_id)->id ?? '' }}">
                        @elseif(Auth::user()->storeable_type === 'App\Models\VsStore')
                            <input type="hidden" name="arcade_id" value="{{ \App\Models\VsStore::find(Auth::user()->storeable_id)->id ?? '' }}">
                        @endif
                    ...
                    <select name="storeable_id" id="storeable_id" 
                        x-model="selectedChip.storeable_id"
                ...
                <div class="w-full mb-3">
                    ...
                    <input type="number" name="revenue_split" id="revenue_split" step="0.05"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
