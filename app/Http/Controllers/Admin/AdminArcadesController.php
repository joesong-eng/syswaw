...
class AdminArcadesController extends Controller
{
    ...
    {
        $id->delete();
        return redirect()->route('admin.arcadeKey.index')->with('success', 'API token deleted successfully.');
    }
    public function keyStore(Request $request)
    {
        $token = $this->createArcadeKey();
        return redirect()->route('admin.arcadeKey.index')->with('success', 'API token created successfully.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validationRules = [
            ...
            'currency' => 'required|string|max:3', // 新增 currency 驗證
            'share_pct' => 'nullable|numeric|min:0|max:100', // 新增：平台分成比例驗證 (0-100之間，可為空)
        ...
        $arcade->share_pct = $validated['share_pct'] ?? null; // 更新平台分成比例，如果未提供則設為 null
