...
class RoleController extends Controller
{
    ...
    public function edit(Role $role){
        ...
        if ($userRoleLevel !== 0 && $role->level <= $userRoleLevel) {
            return redirect()->route('roles.index')->with('error', '無權編輯同級或上級角色');
        }
    ...
    public function destroy(Role $role)
    {
        ...
        // 非 Admin 不能刪除同級或上級角色
        if ($userRoleLevel !== 0 && $role->level <= $userRoleLevel) {
            return redirect()->route('roles.index')->with('error', '無權刪除同級或上級角色');
        }
    ...
    public function show(Role $role)
    {
        ...
        // 非 Admin 不能查看同級或上級角色
        if ($userRoleLevel !== 0 && $role->level <= $userRoleLevel) {
            return redirect()->route('roles.index')->with('error', '無權查看同級或上級角色');
        }
