...
class ArcadesController extends Controller{
    ...
    public function create(){// 查找金鑰，檢查是否過期且是否未使用
 ...
 */
    ...
    //         'role_id' => Role::where('name', 'arcade-staff')->first()->id,
    //         'parent_id' => Auth::id(), // 這樣設置為當前用戶（machine-owner）
    //     ]);
    //             // 指定角色
    //     $manager->assignRole('arcade-staff');
    ...
    public function destroyVsStore($id){ // 查找要刪除的虛擬商店
